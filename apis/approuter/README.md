...
| State Parameter Secret                                                         | `STATE_PARAMETER_SECRET`                        | Controls OAuth CSRF protection via state parameters. **Enabled by default.** Set to `false` to explicitly disable state protection (not recommended). When enabled, the application router creates a state parameter for each initial request. The state parameter value is returned as a session cookie, enabling session stickiness to the same application router instance. After the login callback is executed, this session cookie is replaced by another session cookie. By validating that the authentication server returns the same state parameter in its response, the application router can verify that the response did not originate from a third party. In **Cloud Foundry**, session stickiness is activated automatically. In **Kyma**, you must set up session stickiness manually using a destination rule [(see HTTPCookie)](https://istio.io/latest/docs/reference/config/networking/destination-rule/#LoadBalancerSettings-ConsistentHashLB-HTTPCookie) or another method. The application router stores the state parameter for 60 seconds by default. If the login process is not completed within this time, the request fails with a 401 error. To adjust this time limit, use the `STATE_CACHE` environment variable. |
| PKCE                                                                           | `PKCE_ENABLED`                                  | Enables Proof Key for Code Exchange (PKCE) for the OAuth2 authorization code flow. PKCE is enabled by default. Disabling state protection (setting `STATE_PARAMETER_SECRET=false`) also disables PKCE. Set to `false` to disable PKCE while keeping state protection enabled. |
...
| Backend max sockets                                                            | `BACKEND_MAX_SOCKETS`                           | Maximum number of concurrent TCP connections per backend host for HTTPS keep-alive agents. Default: `100`. Each concurrent TLS connection holds read/write buffers; cap this to prevent OOM under high load. |
...
{
   "oauth2-configuration": {
     "redirect-uris": ["https://*.myapp.<landscapeDomain>/login/callback?authType=ias"],  //Enable redirect after login
     "post-logout-redirect-uris": ["https://*.myapp.<landscapeDomain>/*/logout.html" ]    //Enable redirect after logout
   },
