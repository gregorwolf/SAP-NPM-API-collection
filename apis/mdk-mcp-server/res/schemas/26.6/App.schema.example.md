...
### LinkData Example
For a deeplink URL of "mdkclient://sap.com?ProductId=123&Description=New software&Verified=true#tagExample-1", access the link data using `context.getEventData()`:

```javascript
// OnLinkDataReceived.js
export default function OnLinkDataReceived(context) {
  const eventData = context.getEventData();
  if (eventData) {
    console.log(eventData.eventName);   // 'OnLinkDataReceived'
    console.log(eventData.urlScheme);   // 'mdkclient'
    console.log(eventData.url);         // 'sap.com'
    console.log(eventData.parameters);  // { ProductId: '123', Description: 'New software', Verified: 'true' }
    console.log(eventData.hash);        // '#tagExample-1'
    console.log(eventData.originalURL); // 'mdkclient://sap.com?ProductId=123&Description=New software&Verified=true#tagExample-1'
  }
}
```