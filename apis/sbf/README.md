...
In that case the certificate should be provided in the parameters during bind of the XSUAA _broker_ instance or/and bind of the reuse service instance.
...
The key should be provided (in the environment, for example) to the application (SBF, reuse service or/and consumer application) so that it can be used in combination with the `certificate` from the binding. For SBF the key can be provided through the `SBF_CLIENT_CERTIFICATE_KEY` environment variable or through the `clientCertificateKey` option. Format should be:
